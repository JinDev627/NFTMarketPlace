...
for (i = 0; i < tempnftListArray.length; i++) {
	...
	html += '<button type="button" class="btn btn-success btn_buy" data-bs-toggle="modal" data-bs-target="#saleModal" data-val="' + _nftTokenId + '" data-price-val="' + _price + '"  ">Buy</button> ';
...
$('.btn_buy').click(function () {
	...
	$('.btn_buySubmit').click(async function () {
		...
		if (ownerAddress.toLowerCase() == accounts[0]) {
			alert("The owner can't buy.");
			return false;
		}

		if (!ApprovalState) {
			alert("Change approval  of status");
			return false;
		}
